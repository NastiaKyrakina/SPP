...
import {
    ...
    selectTotal,
    selectUsers,
    selectWorkshops, selectWorkshopsIsLoaded, selectWorkshopsReactions
    ...
    ngOnInit() {
        this.querySubscription = this.route.queryParamMap.subscribe(
            (queryParam: any) => {
                ...
                this.onlyFavorite = category === 'Favorite';
        ...
        this.store.dispatch(new WorkshopReactionRequested({type: 'all'}));
        ...
        this.reactions$ = this.store.pipe(select(selectWorkshopsReactions));
